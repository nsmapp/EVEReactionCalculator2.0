...
class PriceRepoImpl(
    ...
    override suspend fun get(request: PriceRequest, settings: Settings): OrderPrice =
        eveTechRepository.request(request, settings)

    override suspend fun get(request: PriceListRequest, settings: Settings): List<OrderPrice> =
